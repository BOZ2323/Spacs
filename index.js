...
function move(direction) {
    ...
    for (var i = 0; i < arrayOfFullBlocks.length; i++) {
        ...
        switch (direction) {
            case "left":
                blockToMoveInto = currentBlock.blockLeft;
                break;
            case "right":
                blockToMoveInto = currentBlock.blockRight;
                break;
            case "up":
                blockToMoveInto = currentBlock.blockUp;
                break;
            case "down":
                blockToMoveInto = currentBlock.blockDown;
                break;
        }
        ...
        function cannotMoveToNewPosition(currentBlockMove, arrayOfFullBlocks, currentBlock) {
            return currentBlockMove === null || (arrayOfFullBlocks.indexOf(currentBlockMove) !== -1 && currentBlockMove.img !== currentBlock.img)
        }

        if (cannotMoveToNewPosition(blockToMoveInto, arrayOfFullBlocks, currentBlock)) {
            //if it is null, or it is in the arrayOfFullBlocks and the number of currentBlockMove is not the same images
            //i.e. no movement should happen. If the two images are not the same they cannot be merged.
            continue;
        } else if (arrayOfFullBlocks.indexOf(blockToMoveInto) !== -1 && blockToMoveInto.img == currentBlock.img) {
            if (blockToMoveInto.upDated || currentBlock.upDated) {
                continue;
                //no movement happens. If the blocks have already been merged in this move they won't move again.
            }
            blockToMoveInto.upDated = true;
            blockToMoveInto.img = blockToMoveInto.img + 1;
        ...
        blockToMoveInto.img = currentBlock.img;
        arrayOfFullBlocks[i] = blockToMoveInto;
        arrayOfEmptyBlocks[arrayOfEmptyBlocks.indexOf(blockToMoveInto)] = currentBlock;
...
//this draws all the empty blocks:
...
//this draws all the full blocks:
...
drawField();
// the part with "oldPos" happens before the animation has happened
function loopAnimation(arrayToMove, direction) {
    for (var j = 0; j < arrayToMove.length; j++) {
        ...
        for (var i = 0; i < 5; i++) { //this animates the current block 4 times
            setTimeout(function() {
                animate(currBlock, direction)
            }, 20 * i);
        }
        setTimeout(function() { resetLocation(currBlock, oldPos, direction) }, 120); //here it resets 
    }
    //this redraws the grid after the blocks have been moved to their new location
...
function updateLocation(currBlock, direction) {
    switch (direction) {
        case "left":
            currBlock.xPos = currBlock.xPos - 20; //sets the block 20 px to the left
            break;
        case "right":
